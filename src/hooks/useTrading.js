...
export const useTrading = () => {
  ...
  const placeFuturesOrder = async (orderData) => {
    ...
    try {
      ...
      totalPosition = await tradingService.getPositions();
      console.log("totalPosition: ", totalPosition["futuresPositions"]);
      let futuresPositions = totalPosition["futuresPositions"], id = -1;
      if (Array.isArray(futuresPositions)) {
        futuresPositions.forEach((position, index) => {
          console.log("assetType: ", orderData.futuresAssetType, position.assetType);
          console.log("leverage: ", orderData.leverage, position.leverage);
          console.log("limitPrice: ", orderData.limitPrice, position.limitPrice);
          console.log("orderType: ", orderData.orderType, position.orderType);
          console.log("positionType: ", orderData.positionType, position.positionType);
          if(orderData.futuresAssetType == position.assetType && orderData.leverage == position.leverage && orderData.limitPrice == position.limitPrice && orderData.orderType == position.orderType && orderData.positionType == position.positionType){
            console.log("same");
            id = position.id;
            orderData.amount += position.amount;
          }
        });
      } else {
        console.log("futuresPositions is not an array:", futuresPositions);
      }
      
      if(id != -1){
        await tradingService.deletePosition({positionId:id});
        console.log("successful deleted: ", id);
      }
